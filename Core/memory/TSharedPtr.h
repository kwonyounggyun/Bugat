...
public:
    ...
    //shared_ptr로 변환하는 함수. TSharedPtr은 RefCountable을 상속받는 객체만 관리할 수 있으므로, 
    //std::shared_ptr로 변환할 때는 람다가 자신을 복사해서 가지고 들어가 람다 소멸시 해제.
    std::shared_ptr<T> ToSharedPtr() const
    {
        if (_ptr)
        {
            return std::shared_ptr<T>(_ptr, [sptr = *this](T* ptr) mutable {
                sptr.Reset();
                });
        }
        return nullptr;
